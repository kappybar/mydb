...
my_task transaction1(Table *table) {
    ...
    std::optional<std::string> v2 = co_await txn.select("key2");
    if (v2.has_value()) {
        co_await txn.update("key2","value2_new");
    } else {
        co_await txn.insert("key2","value2_new");
    }


    std::optional<std::string> v4 = co_await txn.select("key4");
    if (v4.has_value()) {
        co_await txn.del("key4");
    } else {
        co_await txn.insert("key4","value4_new");
    }
